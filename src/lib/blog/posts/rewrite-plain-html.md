...
things needed to build the static resources. I plan on having this hosted on Cloudflare Pages with a custom domain.

### Problems Encountered
The only problem that I encountered when setting this up was getting the code into Cloudflare Pages. Ultimately
I first tried to create the Pages project through the UI in Cloudflare and execute the Build commands in the makefile to
...
activate the workflow but things still weren't working correctly because the Cloudflare Pages project
needed to be created first. After creating a base placeholder project in Cloudflare things worked as expected.
